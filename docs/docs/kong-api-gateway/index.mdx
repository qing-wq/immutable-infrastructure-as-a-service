...
In IIaaS, [JWT Plugin] is used to authenticate the validity of the access token int the request header. To use it, you
need to enable it in Kong Admin API. Follow the steps below:

1. Enable the plugin globally in order to be effective in all requests and configure the required configuration
   such as `name`:
...
2. Create a consumer. The purpose of creating a consumer is to manage credentials.

```console
curl -X POST http://localhost:8001/consumers -d username=my-consumer
```

3. Create a JWT credential for the consumer. JWT credential is important to validate token. It relies primarily on
three parts in JWT credential:

- `key`: The issuer of JWT token, which is corresponded to the iss field in the payload of JWT token after parsing.
- `algorithm`: The algorithm used to verify the token’s signature.
- `secret` or `rsa_public_key`: Depending on the type of algorithm, decide whether you need to set secret or
...
After do those, all requests with a JWT token of valid signature will be proxied to upstream service.
But if the following conditions occur in the request, it will be rejected and the response status code will be `401`:

- `has no JWT`
- `missing or invalid iss claim`
- `invalid signature`
- `valid signature, invalid verified claim optional`
